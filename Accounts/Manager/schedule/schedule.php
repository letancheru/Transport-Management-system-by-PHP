...
<?php 
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $vcle_sql = "SELECT * FROM `vehicle` WHERE `report` = 'not' ORDER BY plate_no DESC LIMIT 1;";
      $fetch_vcle = mysql_query($vcle_sql);
      $row_vcle = mysql_fetch_array($fetch_vcle);
      if (mysql_num_rows($fetch_vcle) > 0) {

        $plate_no = $row_vcle['plate_no'];
        $vehicle_type=$row_vcle['Vehicle_type'];
       ...
       $drvr_sql = "SELECT * FROM `driver` WHERE `report` = 'not' ORDER BY driver_id DESC LIMIT 1;";
        ...
        if (mysql_num_rows($fetch_drvr) > 0) {
          $driver_id = $row_drvr['driver_id'];
          $driver_name=$row_drvr['firstname']." ".$row_drvr['lastname'];
          $driver_phone=$row_drvr['phone'];
  
        }
    if (isset($plate_no)&&isset($driver_id)&&isset($vehicle_type)&&isset($driver_name)) {
      $sql_VOD = "INSERT INTO `schedule`(`schedule_id`, `date`, `time`, `plate_no`, `vehicle_type`, `driver_id`, `driver_name`,`phone`, `destination`) VALUES('', '$date', '$time', '$plate_no', '$vehicle_type', '$driver_id', '$driver_name','$phone', '$destination');";
      if (mysql_query($sql_VOD)) {        
        mysql_query("UPDATE `vehicle` SET `report`='assigned' WHERE `plate_no` = '$plate_no';");
        mysql_query("UPDATE `driver` SET  `report`='assigned' WHERE `driver_id` = '$driver_id';");
        echo "<script>window.alert('Schedule set Successfully!');</script>";
      }else{
